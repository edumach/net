# Cvičný dynamický web

🌐 Živá ukázka: [https://php.edumach.cz/net/](https://php.edumach.cz/net/)

Pro malé weby, např. webové prezentace živnostníků nebo zájmových spolků může být jednodušší a z hlediska výkonu i následné správy výhodnější nepoužívat rozsáhlý a "velký" redakční systém.

Na druhou stranu psát vše v čistém HTML a ručně kopírovat společné části není komfortní.

Nabízí se třetí cesta: skládání stránek pomocí možností jazyka PHP.

Existuje velmi mnoho různých způsobů a technik, jak takový dynamický web vytvořit. Tento je jen jedním z nich.
...
```
https://tux.panska.cz/~10XPrijmeniJ/net
```
...
V kódu jsou použité ne zcela běžné PHP konstrukce. Nejprve se s nimi důkladně seznamte, abyste kódu rozuměli:
...
$menu = [
    "Domů" => "",
    "Nabídka" => "nabidka",     //zde pribude carka
    "Kontakty" => "kontakty"  //zde uz carka neni
];
```
**(2)** Vytvoření souboru `kontakty.php`. Podmínkou je, že se **musí jmenovat stejně, jako položka v navigaci** (pole `$menu`).

**(3)** Vložení kostry kódu do souboru `kontakty.php`:
```php
<?php
    include "spolecny.php";
    $nazev = "Kontakty"; //stejny text jako v poli $menu
    hlavicka($nazev);
    echo "<h1>$nazev</h1>";
?>
...
Je jako konstanta v souboru `spolecny.php`.


## b) Doplnění stránky `index.php` 

- Krátké představení: čím se firma zabývá, komu poskytuje služby, kde působí – město může být smyšlené (2-3 věty - odstavce `<p>`).
- Jednu větu na závěr, např. o zkušenostech nebo spolehlivosti.


## c) Doplnění stránky `nabidka.php` 

Stránka bude obsahovat:

- Úvodní větu (1–2 věty), že firma nabízí IT služby.

- **Tři až čtyři služby** v odrážkovém seznamu (`<ul>` a `<li>`). Například:
...
* Otevírací dobu ve formě seznamu
